...
static __forceinline  uint32_t av_bswap32(uint32_t x)
{
    return ((((x) << 8 & 0xff00) | ((x) >> 8 & 0x00ff)) << 16 | ((((x) >> 16) << 8 & 0xff00) | (((x) >> 16) >> 8 & 0x00ff)));
}
#   define AV_RB32(p) av_bswap32(*((const __unaligned uint32_t*)(p)))
#   define AV_WB32(p, v) (*((__unaligned uint32_t*)(p)) = av_bswap32(v))
...
{
    return calloc(1, sizeof(struct AVBlowfish));
}
